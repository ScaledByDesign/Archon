...
*   **⚡ Optimized Builds:** Comprehensive Docker caching eliminates re-downloading PyTorch (1.1GB+) on every build - **85% faster rebuilds!**
...
1.  **Build the Docker images with optimized caching:**
    *(First build takes time downloading PyTorch (~1.1GB) and ML dependencies, but subsequent builds are **85% faster** thanks to comprehensive caching!)*
    ...
    **For development with live caching:**
    ```bash
    ...
    *(If you want to customize models/settings in `code/*.py`, do it **before** this step!)*
    ...
    > 💡 **Caching Magic:** The enhanced Dockerfile uses BuildKit cache mounts to persist pip and apt caches between builds. PyTorch wheels and system packages are downloaded only once! See `DOCKER_CACHING_GUIDE.md` for details.
