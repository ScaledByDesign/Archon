...
LLM_START_PROVIDER = "openai"  # Use OpenAI-compatible API for LiteLLM
#LLM_START_MODEL = "qwen3:30b-a3b"
LLM_START_MODEL = "zoi-helper"  # Use LiteLLM model alias from config.yaml
# Alternative models: zoi-thinker, zoi-coder-vllm, zoi-auto
...
from colors import Colors
from simple_mcp_client import simple_mcp_client
...
async def lifespan(app: FastAPI):
    ...
    # Note: Simple MCP client initializes servers on-demand, no bootstrap needed
    logger.info("🎤 Voice-controlled MCP integration ready")
            ...
            if "bytes" in msg and msg["bytes"]:
                ...
                # Add to the handleJSONMessage function in server.py
                    ...
                    if text_content.strip():
                        logger.info(f"🖥️💬 Processing text input: '{text_content[:50]}...'")
                        # Trigger the speech pipeline with the text input
                        app.state.SpeechPipelineManager.prepare_generation(text_content)
                    else:
                        logger.warning("🖥️⚠️ Received empty text_input message.")
