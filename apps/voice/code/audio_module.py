...
class AudioProcessor:
    ...
    def __init__(
        ...
        if engine == "coqui":
            ...
            coqui_models_root = os.getenv('COQUI_TTS_CACHE_DIR', '/models/coqui-tts')
            ensure_lasinya_models(models_root=coqui_models_root, model_name="Lasinya")
            self.engine = CoquiEngine(
                specific_model="Lasinya",
                local_models_path=coqui_models_root,
                voice="reference_audio.wav",
                speed=1.1,
                use_deepspeed=False,  # Disabled for RTX 5070 Ti compatibility (sm_120 architecture not supported by CUDA 12.1)
                thread_count=6,
