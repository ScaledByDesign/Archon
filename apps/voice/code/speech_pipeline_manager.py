...
def parse_tool_call(text: str) -> Optional[Dict[str, Any]]:
    """Parse a potential tool call from LLM output."""
    import json
...
class SpeechPipelineManager:
    ...
    def __init__(
        ...
        # Temporarily disable prewarm to avoid hanging on large models
        # self.llm.prewarm()
    ...
    def _handle_tool_call(self, tool_call_data: dict, gen_id: int) -> str:
        """
        Handle execution of an MCP tool call using simple MCP client.

        Args:
            tool_call_data: Dictionary containing 'tool' and 'arguments' keys
        ...
        try:
            ...
            logger.info(f"🗣️🛠️ [Gen {gen_id}] Voice tool call detected: {tool_name}")

            # Execute tool call using simple MCP client
            # Run async call in event loop (we're in a thread context)
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            try:
                result = loop.run_until_complete(execute_voice_tool_call(tool_call_data))
                return result  # Result is already formatted for voice

            finally:
                loop.close()

        except Exception as e:
            logger.error(f"🗣️🛠️💥 [Gen {gen_id}] Voice tool execution error: {e}")
            ...
            try:
                for chunk in current_gen.llm_generator:
                    ...
                    if not current_gen.quick_answer_provided:
                        # First check if the current response is a tool call
                        tool_call = parse_tool_call(current_gen.quick_answer.strip())
                        if tool_call:
                            logger.info(f"🗣️🛠️ [Gen {gen_id}] Tool call detected during streaming")
                            # Execute the tool and replace the response
                            tool_result = self._handle_tool_call(tool_call, gen_id)
                            current_gen.quick_answer = tool_result
                        ...
                        # Otherwise, check for natural context boundary
                ...
                if not current_gen.llm_aborted and not current_gen.quick_answer_provided:
                    ...
                    tool_call = parse_tool_call(current_gen.quick_answer)
                    if tool_call:
                        logger.info(f"🗣️🛠️ [Gen {gen_id}] Tool call detected in response")
                        # Execute the tool and replace the response
                        tool_result = self._handle_tool_call(tool_call, gen_id)
                        current_gen.quick_answer = tool_result

                    # quick_answer already contains the full text (or tool result)
