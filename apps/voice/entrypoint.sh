...
echo "Entrypoint: Ensuring model directory ownership for user appuser (1001)..." # Informative log.
...
mkdir -p /models/huggingface /models/torch /models/whisper /models/coqui-tts /models/ollama

# Chown command runs as root and should succeed now.
# Uses UID:GID which is robust. Targets both cache and models dirs recursively.
chown -R 1001:1001 /home/appuser/.cache
chown -R 1001:1001 /models

echo "Entrypoint: Initializing models on first run..."
# Initialize models as appuser to ensure proper cache ownership
gosu appuser python /app/code/model_initializer.py
