...
When using with the Zoi platform's Traefik reverse proxy, services are also available via clean domain routing:

| Service            | Domain URL                        | Purpose                           |
| ------------------ | --------------------------------- | --------------------------------- |
| **Web Interface**  | http://archon.zoi.local           | Main dashboard and controls       |
| **API Service**    | http://archon-api.zoi.local       | Web crawling, document processing |
| **MCP Server**     | http://archon-mcp.zoi.local       | Model Context Protocol interface  |
| **Agents Service** | http://archon-agents.zoi.local    | AI/ML operations, reranking       |

**Benefits of Traefik Integration:**
- **Clean URLs**: Professional domain names instead of port numbers
- **SSL Termination**: Automatic HTTPS when certificates are configured
- **Load Balancing**: Built-in health checks and failover support
- **Service Discovery**: Automatic routing updates when services restart

**Usage Examples:**
```bash
# Access the main interface
open http://archon.zoi.local

# API calls via domain
curl http://archon-api.zoi.local/health

# MCP integration via domain
curl http://archon-mcp.zoi.local/mcp
```
