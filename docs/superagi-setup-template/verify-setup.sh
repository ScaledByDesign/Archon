...
litellm_response=$(curl -s -w "%{http_code}" -H "Authorization: Bearer sk-change-me-to-random-string" http://localhost:4000/v1/models)
...
superagi_health=$(curl -s -w "%{http_code}" http://localhost:8001/health 2>/dev/null)
...
api_keys_response=$(curl -s "http://localhost:8001/models_controller/get_api_keys")
